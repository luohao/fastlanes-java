...
public class VectorBytePacker {
	...
	public static void pack0(byte[] input, byte[] output) {
		for (int i = 0; i < 128 / ByteVector.SPECIES_PREFERRED.vectorByteSize(); i++) {
		}
	}
	...
	public static void pack1(byte[] input, byte[] output) {
		for (int i = 0; i < 128 / ByteVector.SPECIES_PREFERRED.vectorByteSize(); i++) {
			ByteVector src;
			ByteVector tmp;
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 0 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src = src.and(MASK1);
			tmp = src;
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 128 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src = src.and(MASK1);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 1));
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 256 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src = src.and(MASK1);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 2));
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 384 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src = src.and(MASK1);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 3));
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 512 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src = src.and(MASK1);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 4));
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 640 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src = src.and(MASK1);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 5));
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 768 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src = src.and(MASK1);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 6));
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 896 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src = src.and(MASK1);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 7));
			tmp.intoArray(output, 0 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
	...
	public static void pack2(byte[] input, byte[] output) {
		for (int i = 0; i < 128 / ByteVector.SPECIES_PREFERRED.vectorByteSize(); i++) {
			...
			src = src.and(MASK2);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 6));
			tmp.intoArray(output, 128 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
			tmp = src.lanewise(VectorOperators.LSHR, 2);
		}
	}
	...
	public static void pack3(byte[] input, byte[] output) {
		for (int i = 0; i < 128 / ByteVector.SPECIES_PREFERRED.vectorByteSize(); i++) {
			...
			src = src.and(MASK3);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 5));
			tmp.intoArray(output, 256 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
	...
	public static void pack4(byte[] input, byte[] output) {
		for (int i = 0; i < 128 / ByteVector.SPECIES_PREFERRED.vectorByteSize(); i++) {
			...
			src = src.and(MASK4);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 4));
			tmp.intoArray(output, 384 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
			tmp = src.lanewise(VectorOperators.LSHR, 4);
		}
	}
	...
	public static void pack5(byte[] input, byte[] output) {
		for (int i = 0; i < 128 / ByteVector.SPECIES_PREFERRED.vectorByteSize(); i++) {
			...
			src = src.and(MASK5);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 3));
			tmp.intoArray(output, 512 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
	...
	public static void pack6(byte[] input, byte[] output) {
		for (int i = 0; i < 128 / ByteVector.SPECIES_PREFERRED.vectorByteSize(); i++) {
			...
			src = src.and(MASK6);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 2));
			tmp.intoArray(output, 640 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
			tmp = src.lanewise(VectorOperators.LSHR, 6);
		}
	}
	...
	public static void pack7(byte[] input, byte[] output) {
		for (int i = 0; i < 128 / ByteVector.SPECIES_PREFERRED.vectorByteSize(); i++) {
			...
			src = src.and(MASK7);
			tmp = tmp.or(src.lanewise(VectorOperators.LSHL, 1));
			tmp.intoArray(output, 768 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
	...
	public static void pack8(byte[] input, byte[] output) {
		for (int i = 0; i < 128 / ByteVector.SPECIES_PREFERRED.vectorByteSize(); i++) {
			ByteVector src;
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 0 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src.intoArray(output, 0 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 128 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src.intoArray(output, 128 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 256 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src.intoArray(output, 256 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 384 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src.intoArray(output, 384 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 512 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src.intoArray(output, 512 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 640 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src.intoArray(output, 640 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 768 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src.intoArray(output, 768 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
			src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, 896 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			src.intoArray(output, 896 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
		}
	}
	...
	public static void unpack1(byte[] input, byte[] output) {
		for (int i = 0; i < 128 / ByteVector.SPECIES_PREFERRED.vectorByteSize(); i++) {
			ByteVector src = ByteVector.fromArray(ByteVector.SPECIES_PREFERRED, input, i * ByteVector.SPECIES_PREFERRED.vectorByteSize());
			ByteVector tmp;
			...
			tmp.intoArray(output, 896 + i * ByteVector.SPECIES_PREFERRED.vectorByteSize());;
		}
	}
