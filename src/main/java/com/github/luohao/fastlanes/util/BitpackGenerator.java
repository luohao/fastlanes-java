...
public class BitpackGenerator
{
    ...
    public static List<MethodSpec> buildPackMethods(Class<?> primitiveType, int unpackedBits)
    {
        ...
        for (int bitWidth = 0; bitWidth <= unpackedBits; bitWidth++) {
            ...
            builder.beginControlFlow("for (int i = 0; i < $L / $T.SPECIES_PREFERRED.vectorByteSize(); i++)", totalLanes, vectorType);
            ...
            if (bitWidth == 0) {
                ...
                for (int row = 0; row < unpackedBits; row++) {
                    builder.addStatement("src = $T.fromArray($T.SPECIES_PREFERRED, input, $L + i * $T.SPECIES_PREFERRED.vectorByteSize())", vectorType, vectorType, index(row), vectorType);
                    builder.addStatement("src.intoArray(output, $L + i * $T.SPECIES_PREFERRED.vectorByteSize());", totalLanes * row, vectorType);
                }
            }
            else {
                builder.addStatement("$T src", vectorType);
                ...
                for (int row = 0; row < unpackedBits; row++) {
                    ...
                    if (next > curr) {
                        builder.addStatement("tmp.intoArray(output, $L + i * $T.SPECIES_PREFERRED.vectorByteSize());", totalLanes * curr, vectorType);
    ...
    public static List<MethodSpec> buildUnpackMethods(Class<?> primitiveType, int unpackedBits)
    {
        ...
        for (int bitWidth = 0; bitWidth <= unpackedBits; bitWidth++) {
            ...
            if (bitWidth == 0) {
                ...
                builder.addStatement("$T src", vectorType);
                for (int row = 0; row < unpackedBits; row++) {
                    builder.addStatement("src = $T.fromArray($T.SPECIES_PREFERRED, input, $L + i * $T.SPECIES_PREFERRED.vectorByteSize())", vectorType, vectorType, totalLanes * row, vectorType);
                    builder.addStatement("src.intoArray(output, $L + i * $T.SPECIES_PREFERRED.vectorByteSize());", index(row), vectorType);
                }
            }
            else {
                builder.addStatement("$T src = $T.fromArray($T.SPECIES_PREFERRED, input, i * $T.SPECIES_PREFERRED.vectorByteSize())", vectorType, vectorType, vectorType, vectorType);
                builder.addStatement("$T tmp", vectorType);
                for (int row = 0; row < unpackedBits; row++) {
                    int curr = (row * bitWidth) / unpackedBits;
                    int next = ((row + 1) * bitWidth) / unpackedBits;
                    ...
                    if (next > curr) {
                        ...
                        if (next < bitWidth) {
                            builder.addStatement("src = $T.fromArray($T.SPECIES_PREFERRED, input, $L + i * $T.SPECIES_PREFERRED.vectorByteSize())", vectorType, vectorType, totalLanes * next, vectorType);
                    ...
                    builder.addStatement("tmp.intoArray(output, $L + i * $T.SPECIES_PREFERRED.vectorByteSize());", index(row), vectorType);
                }
            }
            builder.endControlFlow();
            methods.add(builder.build());
